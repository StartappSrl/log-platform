...
def create_tenant_route():
    ...
    db.session.add(t)
    db.session.commit()
    record_audit("crea_cliente", f"nome={name}")
...
def revoke_certificate_route(serial_number):
    ...
    record_audit("revoca_certificato", f"serial={serial_number}")
...
def create_user_route():
    ...
    db.session.add(u)
    db.session.commit()
    record_audit("crea_utente", f"username={username}, tenant={tenant or '(admin)'}")
...
def toggle_user_active_route(user_id):
    ...
    db.session.commit()
    record_audit("modifica_stato_utente",
                  f"username={u.username}, nuovo_stato={'attivo' if u.is_active else 'disattivato'}")
...
def reset_user_mfa_route(user_id):
    ...
    db.session.commit()
    record_audit("reset_mfa", f"username={u.username}")
    return jsonify(ok=True)
...
def record_audit(action: str, details: str = ""):
    """Registra un'azione amministrativa nel log di controllo - va
    chiamata DOPO che l'azione e' andata a buon fine (db.session.commit()
    gia' fatto), non prima, per non registrare azioni poi fallite."""
    entry = AuditLog(
        username=g.current_user.username if hasattr(g, "current_user") and g.current_user else "sconosciuto",
...
def list_audit_log_route():
    ...
    return jsonify([{
        "username": e.username, "action": e.action, "details": e.details,
        "ip": e.ip, "created_at": e.created_at.isoformat() if e.created_at else None,
    } for e in entries])

@dash.post("/tenants/<name>/delete")
@admin_required
@csrf_protect
def delete_tenant_route(name):
    """Cancella COMPLETAMENTE un cliente: stream e indice Graylog (con i
    dati), tutti i suoi archivi caricati, revoca tutti i suoi
    certificati, disattiva i suoi utenti, e infine cancella il record
    del tenant. Azione irreversibile - richiede che venga ripetuto il
    nome esatto del cliente come conferma, per evitare cancellazioni
    accidentali con un click sbagliato."""
    data = request.get_json(force=True, silent=True) or {}
    ...
    if confirm_name != name:
        return jsonify(error="conferma non corrispondente: riscrivi esattamente il nome del cliente"), 400

    # 1. Graylog: stream + indice (con i dati)
    try:
        if t.graylog_stream_id and t.graylog_index_set_id:
            gl.delete_stream_and_index(t.graylog_stream_id, t.graylog_index_set_id)
    except gl.GraylogError as e:
        return jsonify(error=f"errore Graylog durante la cancellazione: {e} "
                              "- niente e' stato cancellato, riprova"), 502

    # 2. Archivi caricati
    archives_removed = archive_storage.delete_all_archives_for_tenant(name)

    # 3. Certificati (revoca, non cancella - restano nel registro per storico)
    certs_revoked = cert_ledger.revoke_all_certificates_for_tenant(name)

    # 4. Utenti legati a questo tenant: disattivati, non cancellati
    # (per non perdere la storicita' nel log di controllo)
    affected_users = User.query.filter_by(tenant=name).all()
    ...
    record_audit("cancella_cliente",
                  f"nome={name}, archivi_rimossi={archives_removed}, "
                  f"certificati_revocati={certs_revoked}, utenti_disattivati={len(affected_users)}")
