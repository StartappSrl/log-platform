...
def usb_block_loop(stop_flag: dict):
    """Applica il blocco dello storage USB subito, poi lo riafferma ogni
    ora (in caso un amministratore locale lo rimuova a mano) - stesso
    schema di inventory_loop per il controllo di stop_flag e l'attesa."""
    import usb_block
    while not stop_flag["stop"]:
        try:
            msg = usb_block.enforce_usb_block_if_configured(True)
            print(f"[USB storage] {msg}")
        except Exception:
            pass
...
def load_config(config_path: Path):
    ...
    return {
        ...
        "block_usb_storage": s.getboolean("block_usb_storage", fallback=False),
    }
...
def run_agent(stop_flag: dict):
    ...
    if config["block_usb_storage"]:
        threads.append(threading.Thread(
            target=usb_block_loop,
            args=(stop_flag,),
            daemon=True))
